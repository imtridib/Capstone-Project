...
def enrich_with_groq(question: str, raw_answer: str):
    """
    Sends the user's question + the BLIP model's raw (often terse) answer to Groq,
    asking it to reason about what that answer actually means in context and produce
    a properly written response in both English and Bengali. Returns
    (english_text, bengali_text, used_groq: bool).
    Falls back to googletrans if Groq isn't configured or the call fails.
    """
    if groq_client is not None:
        try:
                ...
                messages=[
                    {
                        ...
                        "content": (
                            ...
                            "Respond ONLY with a JSON object with exactly two keys: \"english\" and "
                            "\"bengali\", each a complete answer in that language. No other text."
                        ),
                    },
                    {
                        "role": "user",
                        "content": f"Visitor's question: {question}\nRaw model label: {raw_answer}",
                    },
                ],
                temperature=0.4,
            ...
            bengali = parsed.get("bengali", "").strip()
            if english and bengali:
                return english, bengali, True
        except Exception:
            pass  # fall through to translator-based fallback

    # Fallback: no Groq key configured, SDK missing, or the call failed
    english = raw_answer
    bengali = translator.translate(raw_answer, src="en", dest="bn").text
    return english, bengali, False
...
st.session_state.setdefault("answer", None)  # (english, bengali, used_groq)
...
THEMES = {
    "night": dict(
        ...
        UPLOAD_BG="rgba(212,175,106,0.03)", CARD_BG="rgba(255,255,255,0.015)",
    ...
    "day": dict(
        BG="#FBF7EF", BG_GRAD="radial-gradient(ellipse 80% 60% at 50% -10%, rgba(139,69,19,0.06), transparent)",
        ACCENT="#8B4513", ACCENT_2="#B5541E", ACCENT_DIM="rgba(139,69,19,0.22)",
        ACCENT_MID="rgba(139,69,19,0.45)", ACCENT_FAINT="rgba(139,69,19,0.05)",
        TEXT_PRI="#2C1A0E", TEXT_DIM="rgba(44,26,14,0.55)", TEXT_MONO="rgba(139,69,19,0.6)",
        BORDER="rgba(139,69,19,0.2)", BORDER_FAINT="rgba(139,69,19,0.1)", GRAIN_OP="0.15",
        ANSWER_BG="linear-gradient(135deg, rgba(139,69,19,0.08), rgba(139,69,19,0.02))",
        UPLOAD_BG="rgba(139,69,19,0.03)", CARD_BG="rgba(139,69,19,0.02)",
        HERO_WM="rgba(139,69,19,0.05)", BTN_TEXT="#FBF7EF", BTN_HOVER="#A0522D",
...
<div class="heritage-hero">
    ...
    <h1 class="hero-title">
        Uncover the <em>stories</em> behind<br>Bangladesh's Cultural Sites
    </h1>
        ...
        if image is None:
            ...
            with st.spinner("Reading the image…"):
                ...
                en_question = (
                    ...
                    else translator.translate(question, src=detected_lang, dest="en").text
                )
                raw_answer = run_vqa(image, en_question)

            with st.spinner("Building a contextual answer…"):
                english, bengali, used_groq = enrich_with_groq(en_question, raw_answer)
                st.session_state.answer = (english, bengali, used_groq)

    if st.session_state.answer:
        english, bengali, used_groq = st.session_state.answer
        st.markdown(f"""
        <div class="answer-grid">
            <div class="answer-box">
                <div class="answer-label">Response · English</div>
                <div class="answer-text">{html.escape(english)}</div>
            </div>
            <div class="answer-box">
                <div class="answer-label">উত্তর · বাংলা</div>
                <div class="answer-text">{html.escape(bengali)}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        if used_groq:
